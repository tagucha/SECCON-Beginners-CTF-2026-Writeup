...
## 以下で説明している方法は遅い
他の方のwriteupを見たら、成功確率が`1/255`になっていました。最初に`(C||C)`のような同じブロックが並んだ暗号文の復号を運で通すことで`IV`と`D_K(X)`を同時に得ることができるようです。さすがに成功確率が`(1/255)^2`は低すぎました...。

少し説明すると暗号文`(C||C)`の復号に成功すると
```
P0 = D_K(C) xor IV
P1 = D_K(C) xor C
```
を満たす`P0`と`P1`を得ることができます。これらの値を使うと
```
IV = P0 xor P1 xor C
```
と`IV`を求めることができ、同時に`D_K(C) = P1 xor C`も求められているので、下記で説明している`D_K(X)`を増やしていく方法でチャレンジを1回通すことができます。
残りの三回は、下記の3,4回目のチャレンジの通し方と同じで、暗号化を1回ずつ使いながらやれば良い。
らしいです。頭いい...

[exploit_v2.py](/crypto/Golden_Ticket_2/exploit_v2.py)に改良版を作ってみました。
